...
export default function Patient(){
  ...
  const cancel = (id)=> { api.cancelAppointment(id); location.reload() }
          ...
          <tbody>
            {list.map(a => (
              <tr key={a.id} className="border-t">
                ...
                <td className="p-2">{dayjs(a.start_at).format('DD/MM/YYYY HH:mm')}</td>
                <td className="p-2"><span className="badge">{a.status}</span></td>
                <td className="p-2">{a.payment_status}</td>
                <td className="p-2">
                  {a.status !== 'cancelled' && <button className="btn btn-outline" onClick={()=>cancel(a.id)}>Huỷ</button>}
                </td>
              </tr>
            ))}
            {list.length===0 && (
              <tr><td className="p-4 text-slate-500" colSpan={6}>Chưa có lịch hẹn nào.</td></tr>
            )}
          </tbody>
        </table>
      </div>
